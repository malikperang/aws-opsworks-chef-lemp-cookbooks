...
action :disable do
  ...
  # The nginx.org packages store the default site at /etc/nginx/conf.d/default.conf.erb and our
  # normal script doesn't disable these.
  if new_resource.site_name == 'default' && ::File.exist?('/etc/nginx/conf.d/default.conf.erb') # ~FC023
    execute 'Move nginx.org package default site config to sites-available' do
      command "mv /etc/nginx/conf.d/default.conf.erb #{node['nginx']['dir']}/sites-available/default"
      user 'root'
      notifies :reload, 'service[nginx]'
    end
  end
