...
action :disable do
  ...
  # The nginx.org packages store the default stream at /etc/nginx/conf.d/default.conf.erb and our
  # normal script doesn't disable these.
  if new_resource.stream_name == 'default' && ::File.exist?('/etc/nginx/conf.d/default.conf.erb') # ~FC023
    execute 'Move nginx.org package default stream config to streams-available' do
      command "mv /etc/nginx/conf.d/default.conf.erb #{node['nginx']['dir']}/streams-available/default"
      user 'root'
      notifies :reload, 'service[nginx]'
    end
  end
